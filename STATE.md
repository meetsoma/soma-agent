...
The public-facing GitHub org for **Soma** — an AI coding agent with self-growing memory. Built on Pi, extended with three layers (extensions, skills, rituals). Created by Soma Team, shipped under the Gravicity brand.

Soma is the **reference implementation** of Curtis's protocol inventions (AMP, ATLAS, Three-Layer Model, Breath Cycle, Identity System, Git Identity). Six protocols published at `meetsoma/protocols` (CC BY 4.0). Soma implements them as open source (MIT).
