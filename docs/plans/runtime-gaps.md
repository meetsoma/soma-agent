...
### Problem
Operational protocol frontmatter is heavy. Every `.soma/protocols/*.md` file repeats `author: Soma Team`, `license: MIT`, `scope: shared`, `tier: free` — none of which the runtime reads. That's ~100 tokens per protocol burned on metadata the agent ignores.
...
- Or a comment at the bottom of the file (`<!-- author: Soma Team | MIT | v1.0 -->`)
