...
## Relationship to Published Specs (meetsoma/protocols)

The published CC BY 4.0 specs in `meetsoma/protocols` are the **formal documentation** of protocol concepts. The `.md` files in `.soma/protocols/` are the **operational implementations** — compressed, actionable, agent-readable.

```
meetsoma/protocols/amp/spec.md          → formal spec (for humans, for the community)
